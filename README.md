...
- It also seems like the [*metadata*](#13-metadata--aux-fields) section uses little-endian for its integers.
    - This might be true of any ints as strings regardless of section (the special case string encoded).
...
- `05`: Convert from HEX->Binary: `05`->`00000101`.
    ...
    - Remaining 6 bits are `000101` or `5`, so read the next 5 bytes as UTF-8 (due to non-special format string value type).
...
1. A "magic" string that spells out `REDIS`.
    1. This is represented as HEX->UTF-8: `52 45 44 49 53`->`R E D I S`.
2. An UTF-8 RDB version number, i.e. `0012`.
    1. This is represanted as HEX->UTF-8: `30 30 31 32`->`0 0 1 2`.
...
- `72 65 64 69 73 2d 76 65 72`: The 9 bytes converted to UTF-8 read as -> `redis-ver`.
...
- `37 2e 34 2e 32`: the 5 bytes converted to UTF-8->`7.4.2`.
...
3. Key: `05 68 65 6c 6c 6f`.
    ...
    - Converting HEX->UTF-8: `68 65 6c 6c 6f`->`h e l l o`
4. Value: `05 77 6f 72 6c 64`.
    - `05` HEX->Binary: `05`->`00000101`.
        - First 2 bytes are `00`, thus only the next 6 bits describe size.
        - Remaining bits equal `000101`, or `5`, thus the key comprises the next 5 bytes.
    - Converting HEX->UTF-8: `77 6f 72 6c 64`->`w o r l d`
